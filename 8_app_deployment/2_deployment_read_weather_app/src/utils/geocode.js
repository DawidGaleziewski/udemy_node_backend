...
const getGeocode = (address, callback) => {
  ...
  const geocodeUrl = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
    address
  )}.json?access_token=pk.eyJ1IjoiZGF3aWRnYWxlemlld3NraWRldiIsImEiOiJjazk1eGU2c20wMnkyM2Z0Y2VkbzA2bW03In0.09K6RUNcddaMY0TyUDP4Mw`;

  request({ url: geocodeUrl, json: true }, (error, { body }) => {
    // We want the error part to be re-usable as well adding error to callback
    const {
      features: { length: responseLength },
    } = body;
    if (error) {
