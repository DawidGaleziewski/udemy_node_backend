...
router.post('/users/login', async(req,res) => {
    try {
        // We want to store token generation in a seperate function for re-use
        const user = await User.findByCredentials(req.body.email, req.body.password);
        // as we are working with individual user, and not a collection as a whole it makes sense to access the  method on the user and not User (instance of the model and not the model itself)
        // Code for generating tokens will still be in model
        const token = await user.generateAuthToken();
        // We want to send user and token in one object
        res.send({user, token});
